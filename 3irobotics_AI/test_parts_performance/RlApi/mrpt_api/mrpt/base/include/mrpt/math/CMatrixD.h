...
namespace mrpt
{
	namespace math
	{
		...
		public:
			...
			inline CMatrixD & operator= (const mrpt_Eigen::MatrixBase <OtherDerived>& other) {
				CMatrixTemplateNumeric<double>::operator=(other);
				return *this;
			}
			/*! Constructor from any other Eigen class */
			template<typename OtherDerived>
			inline CMatrixD(const mrpt_Eigen::MatrixBase <OtherDerived>& other) : CMatrixTemplateNumeric<double>(other) { }
			...
			const inline CMatrixD& operator =(const mrpt_Eigen::MatrixBase<Derived>& m) const {
