	...
	const int		timeoutBetween_ms )
{
	...
	while ( alreadyRead<Count && !timeoutExpired )
	{
		...
		{
		    std::cout << "[CClientTCPSocket] Error reading from socket: " << getLastErrorStr().c_str() << std::endl;
		    return 0;
	...
	const int		timeout_ms )
{
	...
	while ( alreadyWritten<Count && !timeoutExpired )
	{
		...
		if( selRet==int(INVALID_SOCKET) )
		{
		    std::cout << "[CClientTCPSocket] Error writing from socket: " << getLastErrorStr().c_str() << std::endl;
            return 0;
		}
...
#endif
	//int res = getsockopt(m_hSock, IPPROTO_TCP, TCP_NODELAY, (char*)&value, (unsigned int *)&length );
	int res = getsockopt(m_hSock, IPPROTO_TCP, TCP_NODELAY, (char*)&value, (socklen_t *)&length );
...
int CClientTCPSocket::getSOSendBufffer()
{
	...
	unsigned int length = sizeof ( value );
#endif
	//getsockopt(m_hSock, SOL_SOCKET, SO_SNDBUF, (char*)&value, (unsigned int *)&length );
	getsockopt(m_hSock, SOL_SOCKET, SO_SNDBUF, (char*)&value, (socklen_t *)&length );
