...
///
void  CParticleFilter::executePredictionUpdate(
	CParticleFilterCapable 			&obj,
	const mrpt::slam::CActionCollection   *action,
	const mrpt::slam::CSensoryFrame	    *observation)
{
...
//////////////////////////
void  CParticleFilter::executeNormalizeResample(CParticleFilterCapable 	&obj_total, TParticleFilterStats *stats )
{
	MRPT_START

    obj_total.normalizeWeights();

	if (!m_options.adaptiveSampleSize &&
		( m_options.PF_algorithm == CParticleFilter::pfStandardProposal ||
		  m_options.PF_algorithm == CParticleFilter::pfOptimalProposal ) )
	{
		if (obj_total.ESS() < m_options.BETA)
		{
			if (m_options.verbose)
				printf_debug("[PF] Resampling particles (ESS was %.02f)\n", obj_total.ESS());
			obj_total.performResampling( m_options );	// Resample
		}
	}
