...
vector<int> get_target(int robot_x, int robot_y) {
	...
	cout << "======== Pooling & Cropping maps =======" << endl;
	double* Ocp_pooling;
	double* Expp_pooling;
	...
	vector<vector<double>> front_map_pool(240, vector<double>(240));
	for (int i = 0; i < 240; i++) {
		for (int j = 0; j < 240; j++) {
	...
	cout << "flag_end = " << flag_end << endl;
	if (flag_end == 0) {
		cout << "There is no frontier left on the map" << endl;
	}

	cout << endl;
	cout << "========== START PREDICTION ==========" << endl;
	vector<float> output_prob(240*240);
	output_prob = predictions(output_maps);

	cout << endl;
	cout << "========== START GETTING TARGET ==========" << endl;
	vector<int> target_point(2);
	target_point = pro_target(output_prob, expand_type, Frontier_crop);
