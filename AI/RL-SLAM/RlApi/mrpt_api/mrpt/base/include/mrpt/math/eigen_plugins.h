...
// -------------------------------------------------------------------------
// Note: This file will be included within the body of mrpt_Eigen::MatrixBase
// -------------------------------------------------------------------------
public:
	...
	EIGEN_STRONG_INLINE Scalar norm_inf() const { return lpNorm<mrpt_Eigen::Infinity>(); }
	...
	EIGEN_STRONG_INLINE void laplacian(mrpt_Eigen::MatrixBase <OtherDerived>& ret) const
	{
	...
	{
#ifdef _DEBUG
		if ((Derived::RowsAtCompileTime!=mrpt_Eigen::Dynamic && Derived::RowsAtCompileTime!=int(row)) || (Derived::ColsAtCompileTime!=mrpt_Eigen::Dynamic && Derived::ColsAtCompileTime!=int(col))) {
		...
		::mrpt::math::detail::TAuxResizer<mrpt_Eigen::MatrixBase<Derived>,SizeAtCompileTime>::internal_resize(*this,row,col);
		...
		float  *out_estimatedResolution = NULL ) const
	{
		...
		do  // Iterative loop:
		{
			mrpt_Eigen::Matrix<Scalar,Derived::RowsAtCompileTime,1> xx = (*this) * x;
	...
	/** Matrix left divide: RES = A<sup>-1</sup> * this , with A being squared (using the mrpt_Eigen::ColPivHouseholderQR method) */
	template<class MAT2,class MAT3 >
	EIGEN_STRONG_INLINE void leftDivideSquare(const MAT2 &A, MAT3 &RES) const
	{
		mrpt_Eigen::ColPivHouseholderQR<PlainObject> QR = A.colPivHouseholderQr();
		if (!QR.isInvertible()) throw std::runtime_error("leftDivideSquare: Matrix A is not invertible");
		RES = QR.inverse() * (*this);
	}

	/** Matrix right divide: RES = this * B<sup>-1</sup>, with B being squared  (using the mrpt_Eigen::ColPivHouseholderQR method)  */
	template<class MAT2,class MAT3 >
	EIGEN_STRONG_INLINE void rightDivideSquare(const MAT2 &B, MAT3 &RES) const
	{
		mrpt_Eigen::ColPivHouseholderQR<PlainObject> QR = B.colPivHouseholderQr();
	...
	// Implemented in eigen_plugins_impl.h (can't be here since mrpt_Eigen::SelfAdjointEigenSolver isn't defined yet at this point.
	...
	{
		mrpt_Eigen::LLT<PlainObject> Chol = derived().template selfadjointView<mrpt_Eigen::Lower>().llt();
		if (Chol.info()==mrpt_Eigen::NoConvergence)
			return false;
		U = PlainObject(Chol.matrixU());
		return true;
	}

	/** Gets the rank of the matrix via the mrpt_Eigen::ColPivHouseholderQR method
	...
	{
		mrpt_Eigen::ColPivHouseholderQR<PlainObject> QR = this->colPivHouseholderQr();
	...
	template <class OtherDerived> EIGEN_STRONG_INLINE void extractRow(size_t nRow, mrpt_Eigen::EigenBase<OtherDerived> &v, size_t startingCol = 0) const {
