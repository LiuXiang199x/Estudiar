...
void CPointsMap::determineMatching2D(
	...
	TMatchingExtraResults & extraResults ) const    ///use this
{
	MRPT_START
	...
	mrpt_Eigen::Array<float,mrpt_Eigen::Dynamic,1>  x_locals(nLocalPoints_4align), y_locals(nLocalPoints_4align);
...
#else
	// Non SSE2 version:
	const mrpt_Eigen::Map<mrpt_Eigen::Matrix<float,mrpt_Eigen::Dynamic,1> > x_org( const_cast<float*>(&otherMap->x[0]),otherMap->x.size(),1 );
	const mrpt_Eigen::Map<mrpt_Eigen::Matrix<float,mrpt_Eigen::Dynamic,1> > y_org( const_cast<float*>(&otherMap->y[0]),otherMap->y.size(),1 );

	mrpt_Eigen::Array<float,mrpt_Eigen::Dynamic,1>  x_locals = otherMapPose.x + cos_phi * x_org.array() - sin_phi *  y_org.array() ;
	mrpt_Eigen::Array<float,mrpt_Eigen::Dynamic,1>  y_locals = otherMapPose.y + sin_phi * x_org.array() + cos_phi *  y_org.array() ;
	...
	const CPose3D *robotPose)
{
	...
	if (IS_CLASS(obs,CObservation2DRangeScan))
	{
        ...
        loadFromRangeScan(*o, &robotPose3D);
