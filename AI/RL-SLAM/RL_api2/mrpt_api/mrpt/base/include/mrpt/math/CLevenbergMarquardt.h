...
namespace math
{
	 ...
	 */
	template <typename VECTORTYPE = mrpt_Eigen::VectorXd, class USERPARAM = VECTORTYPE >
	class CLevenbergMarquardtTempl : public mrpt::utils::CDebugOutputCapable
	{
	public:
		typedef typename VECTORTYPE::Scalar  NUMTYPE;
		typedef mrpt_Eigen::Matrix<NUMTYPE,mrpt_Eigen::Dynamic,mrpt_Eigen::Dynamic>  matrix_t;
			...
			)
		{
			...
			}	else out_info.path = mrpt_Eigen::Matrix<NUMTYPE,mrpt_Eigen::Dynamic,mrpt_Eigen::Dynamic>(); // Empty matrix
