...
int main(){
	...
	vector<int> a(2);
	vector<vector<vector<double>>> mmmap(240, vector<vector<double>>(240, vector<double>(240)));
	vector<vector<double>> mmap(240, vector<double>(240));

	printf("ok1");
	mmmap = get_inputs();
	printf("ok2");
	predictions(mmmap, 1, mmap);
	return 0;
}
