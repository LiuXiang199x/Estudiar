...
/*---------------------------------------------------------------
					getMostLikelyVector
 ---------------------------------------------------------------*/
void CPosePDFParticles::getMostLikelyVector(std::deque<TLikelihoodParam>     &particles_deque)
{
	CParticleList::const_iterator	it=m_particles.begin();
#define DEQUE_SIZE 150
    ...
    for(size_t i = 0; i < DEQUE_SIZE; i++)
    {
        ...
        particles_deque.push_back(temp_param);
    }

	TLikelihoodParam    last_particle;
    last_particle.particle_pose = *it->d;
    last_particle.particle_logw = it->log_w;

	for (++it;it!=m_particles.end();++it)
	{
        double x1 = last_particle.particle_pose.m_coords[0];
        double y1 = last_particle.particle_pose.m_coords[1];
        double x2 = it->d->m_coords[0];
        double y2 = it->d->m_coords[1];
        double distance = sqrt(square(x1 - x2) + square(y1 - y2));

        if(distance < 0.001)
        {
            if(it->log_w > last_particle.particle_logw)
            {
                last_particle.particle_logw = it->log_w;
            }
        }
        else
        {
            ...
            {
                if(last_particle.particle_logw > particles_deque.at(k).particle_logw)
                {
                    for(int j = (DEQUE_SIZE - 1); j >= (k + 1); j--)
                    {
                        particles_deque.at(j) = particles_deque.at(j - 1);
                    }
                    particles_deque.at(k) = last_particle;
                    break;
                }
            }
            last_particle.particle_pose = *it->d;
...
void CPosePDFParticles::getMostLikelyVector(const int                        &deque_size,
                                            std::deque<TLikelihoodParam>     &particles_deque)
{
	CParticleList::const_iterator	it=m_particles.begin();
	CParticleList::const_iterator	head=m_particles.begin();
	particles_deque.clear();
    for(size_t i = 0; i < deque_size; i++)
    {
        TLikelihoodParam   temp_param;
	...
	for (;it!=m_particles.end();++it)
	{
        ...
        if(distance < 0.001 && head!=m_particles.end())
        {
            if(it->log_w > last_particle.particle_logw)
            {
                last_particle.particle_pose = *it->d;
...
/*---------------------------------------------------------------
					getMostLikelyDoubleParticles
 ---------------------------------------------------------------*/
void CPosePDFParticles::getMostLikelyDoubleParticles(CPose2D   &most_pose,
                                                     CPose2D   &surbordinate_pose)
{
	CParticleList::const_iterator	it, itMax=m_particles.begin();
	CParticleList::const_iterator   itSurbordinate=m_particles.begin();
	double		max_w = -1e300;
	...
	most_pose =*itMax->d;
	surbordinate_pose = *itSurbordinate->d;
}

/*---------------------------------------------------------------
					getMostLikelyParticle
 ---------------------------------------------------------------*/
CPose2D	 CPosePDFParticles::getMostLikelyParticle(double   &max_w) const
{
	CParticleList::const_iterator	it, itMax=m_particles.begin();
	max_w = -1e300;
	int count = 0;
	...
	cout << "getMostLikelyParticle--count = " << count << std::endl;
	cout << "getMostLikelyParticle--total_count = " << total_count << std::endl;
