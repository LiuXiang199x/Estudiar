...
namespace math
{
    /** CArrayNumeric is an array for numeric types supporting several mathematical operations (actually, just a wrapper on mrpt_Eigen::Matrix<T,N,1>)
      * \sa CArrayFloat, CArrayDouble, CArray
      */
    template <typename T, std::size_t N>
    class CArrayNumeric : public mrpt_Eigen::Matrix<T,N,1>
    {
	public:
        typedef T                    value_type;
		typedef mrpt_Eigen::Matrix<T,N,1> Base;

    	CArrayNumeric() {}  //!< Default constructor
    	/** Constructor from initial values ptr[0]-ptr[N-1] */
    	CArrayNumeric(const T*ptr) : mrpt_Eigen::Matrix<T,N,1>(ptr) {}
    	...
    	explicit CArrayNumeric(const ARRAYLIKE &obj) : mrpt_Eigen::Matrix<T,N,1>(obj) {}

		template<typename OtherDerived>
		inline CArrayNumeric<T,N> & operator= (const mrpt_Eigen::MatrixBase <OtherDerived>& other) {
			Base::operator=(other);
			return *this;
		}
