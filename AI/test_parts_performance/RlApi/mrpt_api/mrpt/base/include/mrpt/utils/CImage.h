...
namespace mrpt
{
	namespace utils
	{
		...
		public:
			...
			explicit inline CImage(const mrpt_Eigen::MatrixBase<Derived> &m, bool matrix_is_normalized) : img(NULL),m_imgIsReadOnly(false), m_imgIsExternalStorage(false)
			{
			...
			*/
			...
			/** Set the image from a matrix, interpreted as grayscale intensity values, in the range [0,1] (normalized=true) or [0,255] (normalized=false)
			  ...
			  */
			template <typename Derived>
			void setFromMatrix(const mrpt_Eigen::MatrixBase<Derived> &m, bool matrix_is_normalized=true)
			{
				MRPT_START
			...
			/** Set the image from RGB matrices, given the pixels in the range [0,1] (normalized=true) or [0,255] (normalized=false)
			  ...
			  */
			template <typename Derived>
			void setFromRGBMatrices(const mrpt_Eigen::MatrixBase<Derived> &m_r, const mrpt_Eigen::MatrixBase<Derived> &m_g, const mrpt_Eigen::MatrixBase<Derived> &m_b, bool matrix_is_normalized=true)
			{
				MRPT_START
				makeSureImageIsLoaded();   // For delayed loaded images stored externally
				ASSERT_(img);
