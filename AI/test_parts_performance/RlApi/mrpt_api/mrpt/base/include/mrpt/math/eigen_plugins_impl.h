...
//   in the "plug-in" headers "eigen_plugins.h" within mrpt_Eigen::MatrixBase<>
// -------------------------------------------------------------------------
...
namespace internal_mrpt
{
	...
	struct MatOrVecResizer
	{
		...
		static inline void doit(mrpt_Eigen::Matrix<S,R,C,Opt,MaxR,MaxC> &mat, size_t new_rows,size_t new_cols)
		{
			::mrpt::math::detail::TAuxResizer<mrpt_Eigen::Matrix<S,R,C,Opt,MaxR,MaxC>,mrpt_Eigen::Matrix<S,R,C,Opt,MaxR,MaxC>::SizeAtCompileTime>::internal_resize(mat,new_rows,new_cols);
		}
	};
	...
	{
		template <typename S, int Opt, int MaxR, int MaxC>
		static inline void doit(mrpt_Eigen::Matrix<S,R,1,Opt,MaxR,MaxC> &mat, size_t new_rows,size_t new_cols)
		{
			::mrpt::math::detail::TAuxResizer<mrpt_Eigen::Matrix<S,R,1,Opt,MaxR,MaxC>,mrpt_Eigen::Matrix<S,R,1,Opt,MaxR,MaxC>::SizeAtCompileTime>::internal_resize(mat,new_rows);
		}
	};
	...
	{
		template <typename S, int Opt, int MaxR, int MaxC>
		static inline void doit(mrpt_Eigen::Matrix<S,1,C,Opt,MaxR,MaxC> &mat, size_t new_rows,size_t new_cols)
		{
			::mrpt::math::detail::TAuxResizer<mrpt_Eigen::Matrix<S,1,C,Opt,MaxR,MaxC>,mrpt_Eigen::Matrix<S,1,C,Opt,MaxR,MaxC>::SizeAtCompileTime>::internal_resize(mat,new_cols);
		}
	};
	template<>
	...
	{
		template <typename S, int Opt, int MaxR, int MaxC>
		static inline void doit(mrpt_Eigen::Matrix<S,1,1,Opt,MaxR,MaxC> &mat, size_t new_rows,size_t new_cols)
		{
			::mrpt::math::detail::TAuxResizer<mrpt_Eigen::Matrix<S,1,1,Opt,MaxR,MaxC>,mrpt_Eigen::Matrix<S,1,1,Opt,MaxR,MaxC>::SizeAtCompileTime>::internal_resize(mat,new_cols);
		}
	};
}
...
EIGEN_STRONG_INLINE void mrpt_Eigen::MatrixBase<Derived>::eigenVectors( MATRIX1 & eVecs, MATRIX2 & eVals ) const
{
...
EIGEN_STRONG_INLINE void mrpt_Eigen::MatrixBase<Derived>::eigenVectorsVec( MATRIX1 & eVecs, VECTOR1 & eVals ) const
{
	mrpt_Eigen::EigenSolver< Derived > es(*this, true);
...
template <class Derived>
template <class MATRIX1,class MATRIX2>
EIGEN_STRONG_INLINE void mrpt_Eigen::MatrixBase<Derived>::eigenVectorsSymmetric( MATRIX1 & eVecs, MATRIX2 & eVals ) const
{
	Matrix<Scalar,Dynamic,1> evals;
...
template <class Derived>
template <class MATRIX1,class VECTOR1>
EIGEN_STRONG_INLINE void mrpt_Eigen::MatrixBase<Derived>::eigenVectorsSymmetricVec( MATRIX1 & eVecs, VECTOR1 & eVals ) const
{
	// This solver returns the eigenvectors already sorted.
	mrpt_Eigen::SelfAdjointEigenSolver<Derived> eigensolver(*this);
...
template <class Derived>
bool mrpt_Eigen::MatrixBase<Derived>::fromMatlabStringFormat(const std::string &s, std::ostream *dump_errors_here)
{
	// Start with a (0,0) matrix:
	if ( Derived::RowsAtCompileTime==mrpt_Eigen::Dynamic)
		(*this) = Derived();
	...
	while (i<end)
	{
		...
		else
		{
			...
			if ((nRow>0 && size_t(cols())!=N) ||
				(nRow==0 && Derived::ColsAtCompileTime!=mrpt_Eigen::Dynamic && Derived::ColsAtCompileTime!=int(N)) )
			{
				if (dump_errors_here)
			...
			if ( Derived::RowsAtCompileTime==mrpt_Eigen::Dynamic || Derived::ColsAtCompileTime==mrpt_Eigen::Dynamic )
				internal_mrpt::MatOrVecResizer<Derived::RowsAtCompileTime,Derived::ColsAtCompileTime>::doit(derived(),nRow+1,N);
			else if (Derived::RowsAtCompileTime!=mrpt_Eigen::Dynamic && int(nRow)>=Derived::RowsAtCompileTime)
			{
				if (dump_errors_here)
	...
	if (Derived::RowsAtCompileTime!=mrpt_Eigen::Dynamic && int(nRow)!=Derived::RowsAtCompileTime)
	{
		if (dump_errors_here)
...
template <class Derived>
std::string  mrpt_Eigen::MatrixBase<Derived>::inMatlabFormat(const size_t decimal_digits) const
{
	std::stringstream  s;
	s << "[" << std::scientific;
...
template <class Derived>
void mrpt_Eigen::MatrixBase<Derived>::saveToTextFile(
...
template <class Derived>
void mrpt_Eigen::MatrixBase<Derived>::loadFromTextFile(const std::string &file)
{
...
template <class Derived>
void mrpt_Eigen::MatrixBase<Derived>::loadFromTextFile(std::istream &f)
{
	...
	while ( !f.eof() )
	{
		...
		if (str.size() && str[0]!='#' && str[0]!='%')
		{
			...
			// "i": # of columns:
			if ((Derived::ColsAtCompileTime!=mrpt_Eigen::Dynamic && Index(i)!=Derived::ColsAtCompileTime) )
				throw std::runtime_error("loadFromTextFile: The matrix in the text file does not match fixed matrix size");
			if (Derived::ColsAtCompileTime==mrpt_Eigen::Dynamic && nRows>0 && Index(i)!=cols() )
			...
			else if (Derived::RowsAtCompileTime!=mrpt_Eigen::Dynamic && int(nRows)>=Derived::RowsAtCompileTime)
