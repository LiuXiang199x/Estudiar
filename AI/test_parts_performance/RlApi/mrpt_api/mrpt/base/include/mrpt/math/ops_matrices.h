...
namespace mrpt
{
	namespace math
	{
		...
		inline const typename mrpt_Eigen::MatrixBase<Derived>::AdjointReturnType operator ~(const mrpt_Eigen::MatrixBase<Derived> &m) {
			return m.adjoint();
		}

		/** Unary inversion operator. */
		template <class Derived>
		inline typename mrpt_Eigen::MatrixBase<Derived>::PlainObject operator !(const mrpt_Eigen::MatrixBase<Derived> &m) {
			return m.inv();
		}
			...
			bool accumResultInOutput ) //	bool allow_submatrix_mult)
		{
			if (accumResultInOutput)
			     R += ( (H * C.template selfadjointView<mrpt_Eigen::Lower>()).eval() * H.adjoint()).eval().template selfadjointView<mrpt_Eigen::Lower>();
			else
			     R  = ( (H * C.template selfadjointView<mrpt_Eigen::Lower>()).eval() * H.adjoint()).eval().template selfadjointView<mrpt_Eigen::Lower>();
		}
		...
		{
			if (accumResultInOutput)
			     R += ( (H.adjoint() * C.template selfadjointView<mrpt_Eigen::Lower>()).eval() * H).eval().template selfadjointView<mrpt_Eigen::Lower>();
			else
			     R  = ( (H.adjoint() * C.template selfadjointView<mrpt_Eigen::Lower>()).eval() * H).eval().template selfadjointView<mrpt_Eigen::Lower>();
		}
		...
		inline mrpt_Eigen::Matrix<typename MATRIX::Scalar,MATRIX::ColsAtCompileTime,MATRIX::ColsAtCompileTime>
			cov( const MATRIX &v )
		{
			mrpt_Eigen::Matrix<double,MATRIX::ColsAtCompileTime,1> m;
			mrpt_Eigen::Matrix<typename MATRIX::Scalar,MATRIX::ColsAtCompileTime,MATRIX::ColsAtCompileTime>  C;
			meanAndCovMat(v,m,C);
			return C;
		}
