...
namespace mrpt
{
	namespace math
	{
		/**  A matrix of dynamic size.
		  *   Basically, this class is a wrapper on mrpt_Eigen::Matrix<T,Dynamic,Dynamic>, but
		  *   with a RowMajor element memory layout (except for column vectors).
		  *
		  * \note This class exists for backward compatibility of ancient times when MRPT didn't rely on Eigen, feel free to directly use mrpt_Eigen::Matrix<> types instead.
		  *
		...
		class CMatrixTemplateNumeric :
			public mrpt_Eigen::Matrix<
				T,
				mrpt_Eigen::Dynamic,
				mrpt_Eigen::Dynamic,
				// Use row major storage for backward compatibility with MRPT matrices in all cases (even in column vectors!)
				mrpt_Eigen::AutoAlign | mrpt_Eigen::RowMajor
				>
		{
		public:
			typedef mrpt_Eigen::Matrix<T,mrpt_Eigen::Dynamic,mrpt_Eigen::Dynamic,mrpt_Eigen::AutoAlign|mrpt_Eigen::RowMajor> Base;
			...
			*/
			...
			inline CMatrixTemplateNumeric<T>& operator =(const mrpt_Eigen::MatrixBase<Derived>& m) const
			{
				Base::operator =(m);
				return *this;
			}
			...
			template <typename Derived>
			inline bool operator ==(const mrpt_Eigen::MatrixBase<Derived>& m2) const
			{
			...
			template <typename Derived>
			inline bool operator !=(const mrpt_Eigen::MatrixBase<Derived>& m2)  const{ return !((*this)==m2); }
