	...
	/********************************************************************
		...
		********************************************************************/
		...
		if ( reallyInsert )
		{
			...
			// Parameters values:
			const float 	maxDistanceInsertion 	= 8.0;//insertionOptions.maxDistanceInsertion;
			const bool		invalidAsFree			= false; //insertionOptions.considerInvalidRangesAsFreeSpace;
...
Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool  COccupancyGridMap2D::insertFindDockObservation(
		const CObservation	    *obs,
		const CPose3D			*robotPose)
{
#define FRBITS	9
	...
	if ( CLASS_ID(CObservation2DRangeScan)==obs->GetRuntimeClass())
	{
		...
		********************************************************************/
		...
		if (reallyInsert)
		{
			...
			// Parameters values:
			const float 	maxDistanceInsertion 	= 4.0;
			const bool		invalidAsFree			= false;
			float		new_x_max, new_x_min;
			float		new_y_max, new_y_min;
			float		last_valid_range	= maxDistanceInsertion;
			...
			if ( !insertionOptions.wideningBeamsWithDistance )
			{
				...
				for (idx=0;idx<nRanges;idx+=K)
				{
					...
					for (int nStep = 0;nStep<nStepsRay;nStep++)
					{
						updateCell_slow_free(cx,cy, logodd_observation, logodd_thres_free, theMapArray, theMapSize_x );
			...
			else
			{
				...
				for (idx=0;idx<nRanges; idx+=K, A+=dAK)
				{
					...
					if (P0.cy==P2.cy && P0.cy==P1.cy)
					{
						...
						for (int ccx=min_cx;ccx<=max_cx;ccx++)
							updateCell_slow_free(ccx,P0.cy, logodd_observation, logodd_thres_free, theMapArray, theMapSize_x );
					}
					else
					{
						// The intersection point P1b in the segment P0-P2 at the "y" of P1:
						...
						do
						{
							...
							//	last_insert_cx = R1.cx;
								last_insert_cy = R1.cy;
								for (int ccx=R1.cx;ccx<=R2.cx;ccx++)
									updateCell_slow_free(ccx,R1.cy, logodd_observation, logodd_thres_free, theMapArray, theMapSize_x );
							}
...
Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool  COccupancyGridMap2D::motion_insertObservation(
		const CObservation	    *obs,
		const CPose3D			*robotPose)
{
// 	MRPT_START   // Avoid "try" since we use "alloca"
	...
	/********************************************************************
		...
		********************************************************************/
		...
		if ( reallyInsert )
		{
			...
			// Parameters values:
			const float maxDistanceInsertion 	= 3.0;
			const bool  invalidAsFree			= false;
			float		new_x_max, new_x_min;
			float		new_y_max, new_y_min;
			float		last_valid_range	= maxDistanceInsertion;

			float		maxCertainty		= 0.8; //insertionOptions.maxOccupancyUpdateCertainty;
			cellType    logodd_observation  = p2l(maxCertainty);
			cellType    logodd_observation_occupied = 3 * logodd_observation;
			...
			if ( !insertionOptions.wideningBeamsWithDistance )
			{
				...
				for (idx=0, scanPoint_x=scanPoints_x,scanPoint_y=scanPoints_y;idx<nRanges;idx+=K,scanPoint_x++,scanPoint_y++)
				{
					...
					// Add by spencer
					else if(o->validRange[idx] == 2)
					{
						curRange = o->scan[idx];
						float R = std::min(maxDistanceInsertion, curRange);
				...
				for (idx=0;idx<nRanges;idx+=K)
				{
					...
					//  - The ray was not truncated
					// Modify by spencer
					if ( o->validRange[idx] == 1 && o->scan[idx]<maxDistanceInsertion )
						updateCell_fast_occupied(trg_cx,trg_cy, logodd_observation_occupied, logodd_thres_occupied, theMapArray, theMapSize_x );
			...
			else
			{
				...
				for (idx=0;idx<nRanges;idx+=K)
				{
					...
					{
						if (invalidAsFree || o->validRange[idx] == 2)
						{
							// Invalid range:
							float R = min(maxDistanceInsertion,0.5f*last_valid_range);
							scanPoint_x = px + cos(A)* R;
...
/*---------------------------------------------------------------
			Local Map internal insert observation

Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool  COccupancyGridMap2D::local_insertObservation(
		const CObservation	    *obs,
		const CPose3D			*robotPose)
{
#define FRBITS	9
	...
	if ( CLASS_ID(CObservation2DRangeScan)==obs->GetRuntimeClass())
	{
		...
		if ( reallyInsert )
		{
			...
			if (!insertionOptions.wideningBeamsWithDistance )
			{
				...
				if(new_x_max>x_max-securMargen || new_x_min<x_min+securMargen || 
				   new_y_max>y_max-securMargen || new_y_min<y_min+securMargen)
				{
					return false;
				}
...
Insert the observation information into this map.
 ---------------------------------------------------------------*/
bool  COccupancyGridMap2D::static_insertObservation(
		const CObservation	    *obs,
		const CPose3D			*robotPose)
{
// 	MRPT_START   // Avoid "try" since we use "alloca"
	...
	/********************************************************************
		...
		********************************************************************/
		...
		if ( reallyInsert )
		{
			...
			float		maxCertainty		= 0.9; //insertionOptions.maxOccupancyUpdateCertainty;
			cellType    logodd_observation  = p2l(maxCertainty);
			cellType    logodd_observation_occupied = 5 * logodd_observation;
