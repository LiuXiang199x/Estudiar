    ...
    void					*info )
{
	...
	{
	case icpClassic:
		resultPDF = ICP_Method_Classic( m1, mm2, initialEstimationPDF, outInfo );//use this
		break;
...
//			// ----------------------------------------------
//			mrpt_Eigen::Matrix<double,3,mrpt_Eigen::Dynamic> D(3,nCorrespondences);
//
...
//#define ICP_DISTANCES_TO_LINE
				...
				while ( keepIteratingLM &&  ++nLMiters<maxLMiters)
				{
					...
					mrpt_Eigen::VectorXf  dJsq, LM_delta;
					dJ_dq.multiply_Ab( mrpt_Eigen::Map<mrpt_Eigen::VectorXf>(&sq_errors[0],sq_errors.size()), dJsq );
