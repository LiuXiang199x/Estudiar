	...
	for (int i = 0; i < GRID0; i++){
		for (int j = 0; j < GRID0; j++){
			for (int k = 0; k < SPAN; k++){
				...
				float w = exp(data[2]) *anchors[k][0] /352;
				float h = exp(data[3]) *anchors[k][1] /352;
				x = (x +j)/GRID0 -w/2;
				y = (y +i)/GRID0 -h/2;
				boxes[num*4 +0] = x;
				boxes[num*4 +1] = y;
			...
			float* out_pos, float* out_prop, int* out_label)
{
	...
	// int anchors1[SPAN][2] = {{23,27},  {37,58},	   {81,82}};
	// #else
	//ori
	int anchors0[SPAN][2] = {{81,82},  {135,169},  {344,319}};
	...
	// //zangwu yolo
	// int anchors0[SPAN][2] = {{37,15},  {46,35},  {52,49}};
	// int anchors1[SPAN][2] = {{19,13},  {27,17},    {35,28}};
...
//将yolo的两个预测层一维buffer数组改成四维数组类似][k][l][i][j]这样的结构
		...
		for (int j = 0; j < LISTSIZE; j++){
			...
			for (int k = 0; k < GRID0; k++){
				...
				for (int l = 0; l < GRID0; l++){
					//k*GRID0 + l + (i * LISTSIZE + j) * GRID0 * GRID0
					//可以理解为第k行，l列格子的第i个框的第j个值
