...
class GetFrontierMap:
	...
	def __init__(self):
		...
		self.__frontier_pub = rospy.Publisher("/frontier_map", Frontier, queue_size=2)
		self.__end_globalpub = rospy.Publisher("/end_global_cond", String, queue_size=10)
		self.frontiermaps = []
		self.frontiermsg = Frontier()
		self.frontier_exist = False
		self.create_msg_srv()
	...
	def MapsCb(self, globalmapCb):
		...
		if True:
			...
			frontier_map = np.full((self.__height_localmap, self.__width_localmap), 0, dtype=int)
			...
			tmp_frontier = []
			for i in range(len(frontier_map)):
				tmp_frontier.extend(frontier_map[len(frontier_map)-1-i])


			tmp_end_cond = []
			tmp_end_cond.append(score)

			self.frontiermaps = tmp_frontier

			time.sleep(1)

	def get_frontier_map(self):
		return self.frontiermaps
