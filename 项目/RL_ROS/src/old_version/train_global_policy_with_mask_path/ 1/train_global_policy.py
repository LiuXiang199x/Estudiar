...
if __name__ == '__main__':
    ...
    for ep_num in range(num_episodes_start, NUM_GLOBAL_UPDATES):
        ...
        # while env.get_start_signal() == False:
            # continue
        # rospy.loginfo("New env is loaded")
        print("========Antes de nuevo env======")
        # set the env
        env.reset()
        env.global_reset()
        print("========Dentro de nuevo env======") 
        # get observations
        ...
        for step in range(MAX_GLOBAL_STEPS):
            ...
            with torch.no_grad():
                ...
                frontier_map = observations["frontier_map"]  # (bs, M*M)
            ...
            while env.get_path_exist() == False:
                print("why robot doesnt move!!!!!!!!!!!!!")
                print(env.get_path_exist())
                print(env.get_path_error())
                #time.sleep(2)
                env._set_goal(goal_x, goal_y, 1000)
                #if env.get_path_error() == True:
            ...
            if not break_flag:
                ...
                '''
                rospy.loginfo("====== Path lost and Give up ======")
                ...
                length = len(path_x)
                length_realworld = length*0.05
                print("path length(m):", length_realworld)
                
                env.get_path_length(length_realworld, step)
                ...
                current_global_episode_reward += global_rewards  # path_reward
                global_path_length = torch.ones(num_envs, 1)   # path_reward, needs to be changed according to env's output
                global_path_length[0][0] = length
                ...
                local_end = False
                times = 0
                print("--2--:Epoch:{}, step:{}".format(ep_num, step))
                ...
                while int(abs(pose_x - x_coor)) > 4 or int(abs(pose_y - y_coor)) > 4:
                    ...
                    if bumper == -1:
                        ...
                        time.sleep(3)
                        print("--2.5--:Epoch:{}, step:{}".format(ep_num, step))
