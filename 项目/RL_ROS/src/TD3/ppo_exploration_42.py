...
class GlobalPolicy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, prev_actions, masks, deterministic=False):
        ...
        action_probs_mask = torch.where(action_mask == 1, action_logits, torch.ones_like(action_logits) * 1e-9)
        ...
        action_mask = inputs["frontier_mask"]
        action_probs3 = torch.where(action_mask == 1, action_probs, torch.ones_like(action_probs) * 1e-9) #1e-7
...
def _create_global_policy_inputs(global_map, visited_states, map_xy, frontier_map):
    ...
    agent_position_onehot[:, 0, agent_map_x, agent_map_y] = 1
...
if __name__ == '__main__':
    ...
    sl_Process = None
    #from base.tensorboard_utils import TensorboardWriter
    #TENSORBOARD_DIR = os.path.join(os.path.dirname(__file__), "tb_log")
    #with TensorboardWriter(TENSORBOARD_DIR, flush_secs=0.5) as writer:
    ...
    for ep_num in range(num_episodes_start, NUM_GLOBAL_UPDATES):
        ...
        for step in range(MAX_GLOBAL_STEPS):
            ...
            if global_rewards > 8:
                global_rewards += current_global_episode_reward.sqrt() * 4 * 10 - (global_path_length * 10).cpu()
