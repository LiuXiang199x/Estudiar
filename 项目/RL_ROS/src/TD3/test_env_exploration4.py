...
def DeterPoint(map, row, column):
    ...
    return False
    """
    if 0 <= row-1 < map.shape[0]:
        if map[row-1][column] == -1:
            return True
    if 0 <= row+1 < map.shape[0]:
        if map[row+1][column] == -1:
            return True
    if 0 <= column-1 < map.shape[1]:
...
class GazeboEnv:
    ...
    def __init__(self, launchfile, height, width, nchannels):
        ...
        self.episode_num = 1
        self.repeat_num = 5
        self.gz_Process = subprocess.Popen(["roslaunch", "-p", port, fullpath, "gz_world:=TD3-2.world"])  # house_world.world"
    ...
    def step(self, act, sl_Process=None):
    ...
    def reset(self, sl_Process=None):

        self.gz_Process.terminate()

        kill_Process = subprocess.Popen(["killall", "-9", "gzserver gzclient rviz robot_state_publisher move_base slam_gmapping"])
        time.sleep(4.0)
        kill_Process.terminate()
        port = '11311'

        clean_Process = subprocess.Popen(["rosclean", "purge", "-y"])
        time.sleep(1.0)
        clean_Process.terminate()
        self.repeat_num = self.repeat_num - 1
        if self.repeat_num == -1:
            self.repeat_num = 5
            self.episode_num = self.episode_num % 85 + 1 #4 #+= 1
        ...
        print("===========Resetting: unpausing gazebo ")
        ...
        print("===========Resetting: unpaused gazebo")
        print("===========Resetting: waiting for dataodom")
        dataOdom = None
        while dataOdom is None:
            try:
                dataOdom = rospy.wait_for_message('/odom', Odometry, timeout=0.5)
        ...
        state = np.array([self.odomX, self.odomY])
        print("===========Resetting: episode_num, position ", self.episode_num, state)
