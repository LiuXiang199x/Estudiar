...
from test_env_exploration3 import GazeboEnv
...
if __name__ == '__main__':
    ...
    env = GazeboEnv('test_move_base6.launch', 1, 1, 1)
    time.sleep(4)
    port = '11311'
    ...
    if len(checkpoints) == 0:
        ...
        for ep_num in range(num_episodes_start, NUM_GLOBAL_UPDATES):
            ...
            for step in range(MAX_GLOBAL_STEPS):
                ...
                with torch.no_grad():
                    # ================================ act at step
                    global_map = observations["global_map"].to(device)  # (bs, 2, M, M)
                    global_pose = observations["global_pose"].to(device)  # (bs, 3)
                    cur_collison_map = observations["collision_map"].to(device)  # (bs, M, M)
                    cur_visited_map = observations["visited_map"].to(device)  # (bs, 1, M, M),  a path in single-cell size

                    frontier_map = observations["frontier_map"].to(device)  # (bs, M, M)
                        ...
                        for j in [0, 1, -1, 2, -2, 3, -3]:
                            if 0 <= goal_y+i < M and 0 <= goal_x+j < M:
                                if free_space_map[0, goal_y+i, goal_x+j] == 1:
                                    ...
                                    if torch.sum(free_space_map[0, goal_y+i-2:goal_y+i+3, goal_x+j-2:goal_x+j+3], dim=(0, 1)).item() >= 21:
                                        print("=======Goal migrated==========")
                                        break
                                    #break
                        else:
