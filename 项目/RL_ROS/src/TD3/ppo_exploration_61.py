...
from test_env_exploration6 import GazeboEnv
...
class GlobalPolicy(nn.Module):
    def __init__(self, G=150, use_data_parallel=False, gpu_ids=[]):
        ...
        self.actor = nn.Sequential(  # (4, 1200, 1200)
    ...
    def act(self, inputs, rnn_hxs, prev_actions, masks, deterministic=False):
        ...
        free_map = F.max_pool2d(h_12[:, 0:1], kernel_size=8, stride=8) #!!!!!!!!!! needs pooling
        free_mask = free_map.reshape(h_12.shape[0], -1)
    ...
    def evaluate_actions(self, inputs, rnn_hxs, prev_actions, masks, action):
        ...
        value = self.critic(h_12)
        action_probs = dist.probs
        free_map = F.max_pool2d(h_12[:, 0:1], kernel_size=8, stride=8)
        free_mask = free_map.reshape(h_12.shape[0], -1)
        action_probs3 = torch.where(free_mask == 0, action_probs, torch.ones_like(action_probs) * 0) #1e-7
        dist_3 = FixedCategorical(probs=action_probs3, validate_args=False)
        action_log_probs = dist_3.log_probs(action)
    ...
    global_observation_space = spaces.Dict(
        {
            ...
            "frontier_mask": spaces.Box(
                low=0, high=2.0, shape=(M * M,), dtype=np.float32
            ),
        }
    )
...
def _create_global_policy_inputs(global_map, visited_states, map_xy, frontier_map):
    ...
    frontier_mask = frontier_map.view(frontier_map.shape[0], -1)
...
if __name__ == '__main__':
    ...
    overall_map_size = 1200  # 481 # original global map size = 2*M*M
    map_size_for_g_policy = 150 #240
    G = map_size_for_g_policy
    num_envs = 1
    h_rgb = 128
    w_rgb = 128
    ...
    for ep_num in range(num_episodes_start, NUM_GLOBAL_UPDATES):
        ...
        time.sleep(4)
        next_reward_flag = False

        current_global_episode_reward = torch.zeros(num_envs, 1) # explored area
        current_accu_path_length = torch.zeros(num_envs, 1)
        ...
        for step in range(MAX_GLOBAL_STEPS):
            ...
            rospy.loginfo("Selected goal:" + str(goal[0]) + str(' ') + str(goal[1]))
            ...
            current_accu_path_length += global_path_length

            # global_rewards += current_global_episode_reward / 4  # path_reward   ???
            if global_rewards > 8:
                global_rewards += current_global_episode_reward.sqrt() * 2 * 10 - (global_path_length * 10).cpu()

            if next_reward_flag:
                global_rewards = torch.ones(num_envs, 1) * (-500.0)
                next_reward_flag = False
            ...
            if step % NUM_GLOBAL_STEPS == NUM_GLOBAL_STEPS - 1:
                ...
                writer.add_scalar('explored_area', current_global_episode_reward.item(), update)
                writer.add_scalar('path_length', current_accu_path_length.item(), update)
                writer.add_scalar('value_loss', update_metrics["value_loss"], update)
                writer.add_scalar('action_loss:', update_metrics["action_loss"], update)
                writer.add_scalar('dist_entropy:', update_metrics["dist_entropy"], update)
