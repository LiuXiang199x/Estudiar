...
from my_turtlebot2_training.msg import Frontier
from get_frontier_map import GetFrontierMap
import os
...
class TurtleBot2HouseEnv(gym.Env):
    def __init__(self):
        ...
        rospy.Subscriber("/frontier_map", Frontier, self._frontier_map_callback)
        ...
        self.getfrontiermap = GetFrontierMap()
    ...
    def _map_data_callback(self, data):
        ...
        if mapexist == False:
            ...
            for i in range(length):
                if data.data[i] == map_data[i]:
                    a[0] += 1
                if data.data[i] == self.map_data[i]:
                    a[1] += 1
                if self.map_data[i] == map_data[i]:
                    a[2] += 1
            print(a)
            print("len(self.map_data):", len(self.map_data))
            print("len(map_data:)", len(map_data))
            print("len(data.data:)", len(data.data))
            print("=====", data.data == map_data)
            print("=====", data.data == self.map_data)
            print("======:)", self.map_data == map_data)
            '''
            self.map_exist = True
    ...
    def _path_callback(self, data):
        ...
        print("length of path:", len(data.poses))
        ...
        if path_exist == False and search_signal == True and len(data.header.frame_id) != 0:
            ...
            for i in range(len(data.poses)):
                self.path_x.append(data.poses[i].pose.position.x)
                self.path_y.append(data.poses[i].pose.position.y)
    ...
    def get_state(self):
        ...
        frontier_map = self.get_frontiermap()
        M = 481
    ...
    def _frontier_map_callback(self, data):
        self.frontier_map = data.data
        self.frontiermap_exist = True

    def get_frontier_map(self):
        return self.frontier_map

    def get_frontiermap(self):
        """
        Here we get current frontiers map of the turtlebot2
        """
        rospy.logdebug("Start Get Current frontier maps ==>")
        M = 481
        frontier_map = torch.rand(1, M, M).to(device);
        
        for i in range(M):
          for j in range(M): # padded area
            frontier_map[0][i][j] = 1
        
        #while self.frontiermap_exist == False:
        #    continue

        frontiermap = self.getfrontiermap.get_frontier_map()
        map_metadata = self.get_map_metadata()
        ...
        map_H = int(map_metadata.height)
        map_W = int(map_metadata.width)
        length = len(frontiermap)

        map_data_ = numpy.reshape(frontiermap, (map_H,map_W))
        for i in range(map_H):
          for j in range(map_W):
            map_data_[i][j] = frontiermap[j+(map_H-i-1)*map_W]
        origin_x, origin_y = self.world2map(origin_x, origin_y, M, M, 0.1)
        H_d = int(origin_x - map_H)
        W_d = int(origin_y)
        for i in range(map_H):
          for j in range(map_W):
            if i + H_d < M and i + H_d >= 0 and j + W_d < M and j + W_d >= 0:
              if map_data_[i][j] == 0: # free space or small frontier or obstacles
                frontier_map[0][i+H_d][j+W_d] = 0
              elif map_data_[i][j] == 1: # frontiers
                frontier_map[0][i+H_d][j+W_d] = 1
    ...
    def bump_recovery(self, seq):
        ...
        """
        self._set_goal(self.pose[0], self.pose[1], seq)
    ...
    def world2map(self, x_world, y_world, H, W, map_scale):
        ...
        x_map = int(Hby2 - y_world / map_scale)   # index row in globalmap
