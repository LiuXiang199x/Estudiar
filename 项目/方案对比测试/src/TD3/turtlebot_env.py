...
class GazeboEnv:
    ...
    def __init__(self, launchfile, height, width, nchannels):
        ...
        #self.glo_map_sub = rospy.Subscriber('/map',OccupancyGrid, self.map_callback, queue_size=1)
       
    #def map_callback(self, data)
        sl_Process = subprocess.Popen(["roslaunch", "-p", port, "/home/agent/DRL-robot-navigation/TD3/assets/test_move_base5.launch"])

    def get_map(self):
        data = None
        while data is None:
            try:
                data = rospy.wait_for_message('/map', OccupancyGrid, timeout=1.0)
            except:
                pass
        map_data = np.array(data.data)
        #print("bf_reshape:", map_data)
        print("data.info:", map_data.info)
        map_data = map_data.reshape((data.info.width,data.info.height))
        #print("af_reshape:", map_data)
