	...
	TTimeStamp cur_tim)
{
	...
	m_last_loc		= newPose;        //m_last_loc表示的是上一次ICP定位成功的数据
	...
	float w)
{
	...
	{
		const double dT = timeDifference(m_last_odo_time,cur_tim);
		if (dT<=0) std::cout << "[CRobot2DPoseEstimator::processUpdateNewOdometry] WARNING: Diff. in timestamps between odometry should be >0, and it's " << dT << "\n";
...
//        if(new_p.phi > 3.14)
//        {
//            new_p.phi = 3.14 - new_p.phi;
//            int a;
//            cin >> a;
//        }
//        else if(new_p.phi < -3.14)
//        {
//            new_p.phi = 6.28 + new_p.phi;
//            int a;
//            cin >> a;
//        }
	}
