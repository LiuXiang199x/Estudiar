...
namespace slam
{
    ...
    public:
        /* Kimbo add, get map ptr */
        signed char* getMapPtr() {return (signed char*)(&map[0]);}
	std::vector<cellType>::iterator getPMap() {return map.begin(); }
	CObservation2DRangeScan m_simulatedObs;
	...
	protected:
		...
		/** These are auxiliary variables to speed up the computation of observation likelihood values for LF method among others, at a high cost in memory (see TLikelihoodOptions::enableLikelihoodCache).
		  */
		std::vector<float>		precomputedLikelihood;
        ...
        double	 computeObservationLikelihood_likelihoodField_Consensus(
					const CObservation		*obs,
					const CPose2D				&takenFrom,
                    double                      &min_logw);
...
//		 virtual bool  internal_insertObservation( const CObservation *obs, const CPose3D *robotPose = NULL );
	...
	public:
        ...
        void  enableMap() {m_is_empty = false;}

		virtual bool  internal_insertObservation( const CObservation *obs, const CPose3D *robotPose = NULL );

		virtual bool  motion_insertObservation( const CObservation *obs, const CPose3D *robotPose = NULL);
		virtual bool  local_insertObservation( const CObservation *obs, const CPose3D *robotPose = NULL );
		virtual bool  static_insertObservation( const CObservation *obs, const CPose3D *robotPose = NULL);

		virtual bool  insertFindDockObservation( const CObservation *obs, const CPose3D *robotPose = NULL );
		...
		inline void   setCell_int(int x,int y,int value)
		{
			// The x> comparison implicitly holds if x<0
			if (static_cast<unsigned int>(x)>=size_x ||	static_cast<unsigned int>(y)>=size_y)
					return;
			else	map[x+y*size_x] = value;
		}

		inline void   setCellOnly(int x,int y,float value)
		{
			map[x+y*size_x]=p2l(value);
		}
		...
		/** Read the real valued [0,1] contents of a cell, given its index.
		 */
		inline float  getCellOnly(int x,int y) const
		{
            return l2p(map[x+y*size_x]);
		}
		...
		inline int  getCell_int(int x,int y) const
		{
			// The x> comparison implicitly holds if x<0
			if (static_cast<unsigned int>(x)>=size_x ||	static_cast<unsigned int>(y)>=size_y)
					return 0;
			else	return map[x+y*size_x];
		}
		...
		*/
		...
		public:
		  ...
		  */
		 void  subSample( int downRatio, float obstacle_value = 0.2);

         /** Performs a downsampling of the gridmap, by a given factor: resolution/=ratio
             add by shizhe
		  */
         void  sample( int downRatio );

         /** Performs a downsampling of the gridmap, by a given factor: resolution/=ratio
             add by shizhe
		  */
         void  sampleBymin( int downRatio );
	...
	public:
		...
		void  laserScanSimulatorOpt(
		 ...
		 bool  sparialSearch(int &initial_idx,  int &initial_idy,  int &final_idx,    int &final_idy) const;
        ...
        /** Add by Spencer
          */
        bool IsNeighborFree(const int    current_idx,
		...
		double	 computeObservationLikelihood( const CObservation *obs, const CPose2D &takenFrom, double &min_logw);
		...
		double	 computeLikelihoodField_Thrun( const CPointsMap	*pm, double &min_logwconst, const CPose2D *relativePose = NULL);
		double	 computeLikelihoodField_Consensus( const CPointsMap	*pm, double &min_logwconst, const CPose2D *relativePose = NULL);

		void  precomputedLikelihood_Thrun(const int            &x_min,
                                          const int            &y_min,
                                          const int            &x_max,
                                          const int            &y_max);

        ///sara add
		std::vector<float>       m_precomputed_coarse;
		std::vector<float>       m_precomputed_fine;

        int getMapSize() {return map.size();}

        void setPrecomputedCoarseVector(const std::vector<float>  &temp_vector);
        void setPrecomputedFineVector(const std::vector<float>  &temp_vector);

        const std::vector<float> &getPrecomputedCoarseVector();
        const std::vector<float> &getPrecomputedFineVector();

        void precomputedLikelihood_coarse(const int            &x_min,
                                        const int            &y_min,
                                        const int            &x_max,
                                        const int            &y_max,
                                        std::vector<float>   &likelihood);

        void precomputedLikelihood_fine(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse1(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse2(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse3(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse4(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse5(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse6(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse7(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_coarse8(const int            &x_min,
                                            const int            &y_min,
                                            const int            &x_max,
                                            const int            &y_max,
                                            std::vector<float>   &likelihood);

        void precomputedLikelihood_fine1(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine2(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine3(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine4(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine5(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine6(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine7(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        void precomputedLikelihood_fine8(const int            &x_min,
                                         const int            &y_min,
                                         const int            &x_max,
                                         const int            &y_max,
                                         std::vector<float>   &likelihood);

        size_t getPrecomputedVectorSize() {return precomputedLikelihood.size();}
        void updatePrecomputedVector2InputVector(std::vector<double>    &input_vector,
                                                 int input_xx_min, int input_xx_max,
                                                 int input_yy_min, int input_yy_max);
        void setPrecomputedVector(const std::vector<float>    &temp_vector);
        void setPrecomputedVector2(const std::vector<float>    &temp_vector);
        const std::vector<float> &getPrecomputedVector();
        void clearPrecomputedVector();
	    void releasePrecomputeVectorCache();
