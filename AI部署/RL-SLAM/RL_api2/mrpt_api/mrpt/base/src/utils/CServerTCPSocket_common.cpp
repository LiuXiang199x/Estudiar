	...
	int               maxConnectionsWaiting )
{
	...
	// Release tcp port--add by Kimbo
	// ----------------------------
    const int reuse = 1;
    if( int(INVALID_SOCKET) == setsockopt(m_serverSock, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse, sizeof(reuse)) )
        THROW_EXCEPTION( getLastErrorStr() );
...
//	desiredIP.sin_addr.s_addr	= inet_addr( IPaddress.c_str() );
	desiredIP.sin_addr.s_addr	= INADDR_ANY;                   // change by Kimbo---use self ip
