...
//     add by Kimbo, suport interface to set map update and get map update flag
    if(m_close_map_update)
    {
        do_map_update = false;
    }

    m_map_update_flag = do_map_update;
...
/*---------------------------------------------------------------
					initialize
  ---------------------------------------------------------------*/
void CMetricMapBuilderRBPF::setmetricMapBuilderPose(mrpt::poses::CPose3D pose, mrpt::system::TTimeStamp time)
{
    int i = 0;
    for (CMultiMetricMapPDF::CParticleList::iterator it = mapPDF.m_particles.begin(); it!=mapPDF.m_particles.end(); ++it)
    {

        it->d->robotPath.push_back(pose);
        i++;
    }
}
